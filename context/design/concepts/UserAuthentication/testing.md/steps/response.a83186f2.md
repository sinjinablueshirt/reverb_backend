---
timestamp: 'Sat Oct 11 2025 12:56:12 GMT-0400 (Eastern Daylight Time)'
parent: '[[../20251011_125612.9d49594e.md]]'
content_id: a83186f209db2f6732226d060ada74547e827a80a8db94d66be99fedd56c7c51
---
  ...
  try {
    ...
    await t.step(
      ...
      async () => {
        ...
        assert(!("user" in result), "Login with wrong password should not return a user.");
      },
    );
    ...
    await t.step(
      ...
      async () => {
        ...
        // Check if the login failed with an error
        assertObjectMatch(result, { error: "Invalid username or password." });
        assert(!("user" in result), "Login with wrong username should not return a user.");
      },
    );
    ...
    await t.step(
      ...
      async () => {
        ...
        // Check if the login failed with an error
        assertObjectMatch(result, { error: "Invalid username or password." });
        assert(!("user" in result), "Login for non-existent user should not return a user.");
      },
    );
