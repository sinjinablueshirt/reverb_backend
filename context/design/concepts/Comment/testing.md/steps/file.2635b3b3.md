...
content_id: 2635b3b3df77fbe77b526b73136d9e34c4abf02dfb2ec75760cbb00c8395d733
---
...
Deno.test("CommentConcept: Register, AddComment, and RemoveComment functionality", async (test) => {
  ...
  await test.step("1. Operational Principle: Register a resource, add multiple comments, and delete one", async () => {
    ...
    console.log(
      `  Output: Registration result for ${resourceA}: ${
    ...
    console.log(
      `  Verification: Checking initial state of resource ${resourceA}`,
    );
    ...
    console.log(
      `  Action: addComment(resource: ${resourceA}, commenter: ${user1}, text: "First comment!", date: 2023-01-01)`,
    );
    ...
    console.log(
      `  Action: addComment(resource: ${resourceA}, commenter: ${user2}, text: "Second comment.", date: 2023-01-01)`,
    );
    ...
    console.log(
      `  Verification: Comment IDs ${commentId1} and ${commentId2} are unique.`,
    );

    // Verify resourceA now has both comments associated
    console.log(
      `  Verification: Checking resource ${resourceA} for added comments.`,
    );
    ...
    console.log(
      `  Action: removeComment(comment: ${commentId1}, user: ${user1})`,
    );
    ...
    console.log(
      `  Verification: Checking state after removing comment ${commentId1}.`,
    );
    storedResourceA = await concept.resources.findOne({ _id: resourceA });
    assertEquals(
      storedResourceA?.comments.length,
  ...
  await test.step("2. Scenario: Attempting to register an already registered resource", async () => {
    ...
    console.log(
      `  Setup: Registering resource ${resourceA} for the first time.`,
    );
    ...
    console.log(
      `  Verification: Checking state of resource ${resourceA} after failed re-registration.`,
    );
    storedResourceA = await concept.resources.findOne({ _id: resourceA });
    assertEquals(
      storedResourceA?.comments.length,
  ...
  await test.step("3. Scenario: Adding a comment to an unregistered resource", async () => {
    ...
    console.log(
      `  Action: addComment(resource: ${resourceB} (unregistered), commenter: ${user1}, text: "Comment on unregistered.", date: now)`,
    );
    ...
    console.log(
      `  Verification: Checking comments collection and resource ${resourceB}.`,
    );
    ...
    console.log(
      `  State: Comments collection has ${commentsCount} documents (expected 0).`,
    );
    console.log(
  ...
  await test.step("4. Scenario: Multiple comments from the same user on the same resource", async () => {
    ...
    console.log(
      `  Action: addComment(resource: ${resourceA}, commenter: ${user1}, text: "First thought.", date: ${date3})`,
    );
    ...
    console.log(
      `  Action: addComment(resource: ${resourceA}, commenter: ${user1}, text: "Second thought.", date: ${date4})`,
    );
    ...
    console.log(
      `  Verification: Comment IDs ${commentId3} and ${commentId4} are distinct.`,
    );
  ...
  await test.step("5. Scenario: Adding comments with identical content creates distinct entries", async () => {
    ...
    console.log(
      `  Action: addComment(resource: ${resourceA}, commenter: ${user1}, text: "${commonText}", date: ${commonDate}) (first instance)`,
    );
    ...
    console.log(
      `  Action: addComment(resource: ${resourceA}, commenter: ${user1}, text: "${commonText}", date: ${commonDate}) (second instance)`,
    );
  ...
  await test.step("6. Scenario: removeComment precondition failures and successful removal", async () => {
    ...
    console.log(
      `  Setup State: Comment ID ${commentIdToDelete} added for removal tests.`,
    );

    // Precondition 1 failure: `comment` does not exist
    const nonExistentCommentId = freshID();
    console.log(
      `  Action: Attempting to remove non-existent comment ${nonExistentCommentId} by user ${user1}.`,
    );
    ...
    console.log(
      `  Verification: Checking state after failed removal of non-existent comment.`,
    );
    let storedResourceA = await concept.resources.findOne({ _id: resourceA });
    assertEquals(
      storedResourceA?.comments.length,
    ...
    console.log(
      `  Action: Attempting to remove comment ${commentIdToDelete} by wrong user ${user3}.`,
    );
    ...
    console.log(
      `  Verification: Checking state after failed removal by wrong user.`,
    );
    storedResourceA = await concept.resources.findOne({ _id: resourceA });
    assertEquals(
      storedResourceA?.comments.length,
    ...
    console.log(
      `  Action: Attempting successful removal of comment ${commentIdToDelete} by correct user ${user1}.`,
    );
