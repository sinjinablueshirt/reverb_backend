...
content_id: ff61768919dfead253887ed42e302ab05688e3e967207e69f1f0c6547c5f8f5f
---
...
export default class FileUrlConcept {
  ...
  async uploadFile(
    ...
    if (existingFile) {
      return {
        error: `File '${filePath}' already uploaded by owner '${owner}'.`,
      };
    }
    ...
    try {
      ...
      return { error: `Failed to read local file: ${readError}` };
    }
    ...
    try {
      ...
      if (!insertResult.acknowledged) {
        ...
        console.error(
          "MongoDB insert not acknowledged for new file:",
          newFileId,
        );
      ...
      return {
        error: `Failed to upload file to cloud storage: ${gcsError}`,
      };
    }
  }
