---
timestamp: 'Sat Oct 18 2025 14:03:32 GMT-0400 (Eastern Daylight Time)'
parent: '[[../20251018_140332.d85bc43a.md]]'
content_id: c33066a36f9c8cb608f12bf216c04f37fd033d14b213bc5a8af3012c8137e89e
---

# response:

To augment the `FileUrlConcept` implementation to handle `fileData` (as a `Blob`) directly from a front-end and manage Google Cloud Storage interactions entirely within the concept, here's the updated code.

**Key Changes:**

1. **`FileDocument` Interface Update:** The `filePath` field is removed, and `originalFileName` is added to store the user-provided file name. The `fileData` type parameter from the concept's state is implicitly handled by the `gcsObjectName` and `url` fields, which point to the actual data in GCS.
2. **`uploadFile` Action Signature:** The action now accepts `fileData` (a `Blob` representing the file's content) and `originalFileName` (a `string` for the file's name), along with the `owner`.
3. **GCS Upload Logic:**
   * The `Deno.stat(filePath)` check is removed since we're no longer dealing with local file paths.
   * The `Blob` object is converted to a `Uint8Array` before being passed to Google Cloud Storage's `save` method.
   * A uniqueness check based on `owner` and `originalFileName` is added (this is a design choice to prevent logical duplicates, and can be adjusted).
   * `contentType` is set based on `Blob.type`.
4. **Error Handling:** Consistent `{ error: string }` return for non-exceptional failures.
5. **Documentation:** JSDoc comments are updated to reflect the new `uploadFile` signature and logic.
...
export default class FileUrlConcept {
   ...
   */
  async uploadFile(
    { fileData, originalFileName, owner }: { fileData: Blob; originalFileName: string; owner: User },
    ...
    try {
      ...
      await this.storage.bucket(this.bucketName).file(gcsObjectName).save(fileContentBuffer, {
        metadata: {
