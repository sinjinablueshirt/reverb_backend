---
timestamp: 'Wed Oct 15 2025 22:41:18 GMT-0400 (Eastern Daylight Time)'
parent: '[[../20251015_224118.1d5ba64f.md]]'
content_id: ce311db34cc0ac31cdd20d5cebb9d97e51a77c639320e0aa7c623cad6297fe9d
---
...
export default class FileUrlConcept {
  ...
  constructor(private readonly db: Db) {
    ...
    if (!bucketName) {
      ...
      throw new Error("GCS_BUCKET_NAME environment variable not set. Please configure your .env file.");
    }
   ...
   */
  async uploadFile({ filePath, owner }: { filePath: string; owner: User }): Promise<{ file: File } | { error: string }> {
    ...
    try {
      ...
      return { error: `Failed to access local file at path '${filePath}': ${e.message}` };
    }

    // REQUIREMENT CHECK 2: A file with this `filePath` isn't already uploaded by this `owner`
    const existingFile = await this.files.findOne({ filePath: filePath, owner: owner });
    if (existingFile) {
      return { error: `File '${filePath}' has already been uploaded by owner '${owner}' (File ID: ${existingFile._id}).` };
    }
    ...
    try {
      ...
      return { error: `Failed to upload file to Google Cloud Storage: ${gcsError.message || gcsError}.` };
    }
  }
